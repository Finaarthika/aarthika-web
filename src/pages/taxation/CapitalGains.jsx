...
export default function CapitalGains() {
  ...
  const handleCapitalChange = (type, q, val) => {
    updateNestedTaxData('capitalGains', type, q, Number(val));
  };

  const totalStcg = Object.values(capitalGains.stcg).reduce((a,b) => a+b, 0);
  const totalLtcg = Object.values(capitalGains.ltcg).reduce((a,b) => a+b, 0);
